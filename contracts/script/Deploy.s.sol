...
contract Deploy is Script {
    ...
    function run() public {
        ...
        (address factory, address template) = deployContracts(network);

        vm.stopBroadcast();

        // Log deployment results
        logDeploymentResults(network, factory, template);

        // Save deployment info to file
        saveDeploymentInfo(network, factory, template);

        // Verify contracts if needed
        if (!isLocalNetwork(network)) {
            logVerificationInfo(factory, template);
        }
    }

    function deployContracts(string memory network) internal returns (address factory, address template) {
        ...
        // 1. Deploy ERC20Template implementation
        ...
        factoryContract.addTemplate(factoryContract.BASIC_ERC20(), template);
        factoryContract.addTemplate(factoryContract.MINTABLE_ERC20(), template);
        factoryContract.addTemplate(factoryContract.FULL_FEATURED(), template);
        
        console.log("   Templates configured successfully");

        // 6. Verify deployment
        console.log("\n5. Verifying deployment...");
        verifyDeployment(factory, template, config);

        console.log("Deployment completed successfully!");
        return (factory, template);
    }

    function verifyDeployment(address factory, address template, DeployConfig memory config) internal view {
        TokenFactory factoryContract = TokenFactory(factory);
        ...
        // Verify templates
        require(factoryContract.getTemplate(factoryContract.BASIC_ERC20()) == template, "Invalid basic template");
        require(factoryContract.getTemplate(factoryContract.MINTABLE_ERC20()) == template, "Invalid mintable template");
        require(factoryContract.getTemplate(factoryContract.FULL_FEATURED()) == template, "Invalid full featured template");
    ...
    function logDeploymentResults(string memory network, address factory, address template) internal view {
        TokenFactory factoryContract = TokenFactory(factory);
        ...
        console.log("ERC20Template:", template);
    ...
    function logVerificationInfo(address factory, address template) internal view {
        console.log("\n=== VERIFICATION INFO ===");
        ...
        console.log("3. ERC20Template:", template);
        console.log("\nUsing Foundry:");
    ...
    function saveDeploymentInfo(string memory network, address factory, address template) internal {
        ...
        vm.serializeAddress(json, "template", template);
    ...
    function deployLocal() public returns (address factory, address template) {
        vm.startBroadcast();
        (factory, template) = deployContracts("localhost");
        vm.stopBroadcast();
        return (factory, template);
    }
    ...
    function deployWithConfig(
        ...
        address feeRecipient,
        uint256 serviceFee
    ) public returns (address factory, address template) {
        ...
        vm.startBroadcast();
        (factory, template) = deployContracts("custom");
        vm.stopBroadcast();
        return (factory, template);
    }
    ...
    function deployForXsc() public returns (address factory, address template) {
        ...
        (factory, template) = deployContracts("xsc");
